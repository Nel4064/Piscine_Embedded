...
void init_adc(void)
{
	...
	// Set Prescaler 8 (2MHz ADC clock)
	// DS40002061B-page 249 => By default, the successive approximation circuitry requires an input clock frequency between 50kHz and 200kHz to get maximum resolution.
	ADCSRA &= ~(1 << ADPS2); // DS40002061B-page 259
	ADCSRA |= (1 << ADPS1);
	ADCSRA |= (1 << ADPS0);
...
void init_timer1(void)
{
	...
	// TOP = F_CPU / (F_CTC * prescaler) - 1 = 16,000,000 / (64 * 150) - 1 = 1665.66
	OCR1A = 1666;
...
void	ADC_vect()
{
	...
	static volatile uint16_t	data_set[3];
	...
	if (count < 3)
	{
		// Store the current ADC reading in the short term data storage
		data_set[count] = ADCL | (ADCH << 8);
		// => DS40002061B-page 260 ADCL must be read first, then ADCH.
	...
	if (count == 2)
	{
		...
		{
			uart_tx_uint10(data_set[i]);
			// uart_tx("0123456789abcdef"[(data_set[i] & 0x0300) >> 8]);
			// uart_tx("0123456789abcdef"[(data_set[i] & 0x00F0) >> 4]);
			// uart_tx("0123456789abcdef"[(data_set[i] & 0x000F) >> 0]);
			if (i != 2)
			{
				uart_tx(',');
				uart_tx(' ');
